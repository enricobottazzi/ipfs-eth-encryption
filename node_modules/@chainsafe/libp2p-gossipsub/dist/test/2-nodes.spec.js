...
async function nodesArePubSubPeers(node0, node1, timeout = 60000) {
    await pWaitFor(() => {
        const node0SeesNode1 = node0
            .getPubSub()
            .getPeers()
            .map((p) => p.toString())
            .includes(node1.getPeerId().toString());
        const node1SeesNode0 = node1
            .getPubSub()
            .getPeers()
            .map((p) => p.toString())
            .includes(node0.getPeerId().toString());
...
describe('2 nodes', () => {
    ...
    describe('subscription functionality', () => {
        ...
        it('Subscribe to a topic', async () => {
            ...
            expect(nodes[0]
                ...
                .map((p) => p.toString())).to.include(nodes[1].getPeerId().toString());
            expect(nodes[1]
                .getPubSub()
                .getSubscribers(topic)
                .map((p) => p.toString())).to.include(nodes[0].getPeerId().toString());
            ...
            expect(nodes[0].getPubSub().mesh.get(topic)?.has(nodes[1].getPeerId().toString())).to.be.true();
            expect(nodes[1].getPubSub().mesh.get(topic)?.has(nodes[0].getPeerId().toString())).to.be.true();
        });
    });
    describe('publish functionality', () => {
        ...
        it('Publish to a topic - nodeA', async () => {
            const promise = pEvent(nodes[1].getPubSub(), 'message');
        ...
        it('Publish to a topic - nodeB', async () => {
            const promise = pEvent(nodes[0].getPubSub(), 'message');
    ...
    describe('publish after unsubscribe', () => {
        ...
        it('Unsubscribe from a topic', async () => {
            ...
            const evt = await pEvent(nodes[1].getPubSub(), 'subscription-change');
