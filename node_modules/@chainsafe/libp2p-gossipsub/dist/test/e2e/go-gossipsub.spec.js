 ...
 */
const checkReceivedMessage = (topic, data, senderIx, msgIx) => async (node, receiverIx) => await new Promise((resolve, reject) => {
    const t = setTimeout(() => {
...
describe('go-libp2p-pubsub gossipsub tests', function () {
    ...
    it('test gossipsub fanout expiry', async function () {
        ...
        await Promise.all(sendRecv);
        expect(psubs[0].getPubSub().fanout).to.not.be.empty();
        await pWaitFor(async () => {
            return psubs[0].getPubSub().fanout.size === 0;
        });
    });
    ...
    it('test gossipsub gossip propagation', async function () {
        ...
        const results = Promise.all(group2.slice(1).map(async (ps, ix) => new Promise((resolve, reject) => {
            ...
            ps.getPubSub().addEventListener('message', (e) => {
    ...
    it('test gossipsub tree topology', async function () {
        ...
        */
        ...
        expect(psubs[0]
            .getPubSub()
            .getPeers()
            .map((s) => s.toString())).to.have.members([psubs[1].getPeerId().toString(), psubs[5].getPeerId().toString()]);
        expect(psubs[1]
            .getPubSub()
            .getPeers()
            .map((s) => s.toString())).to.have.members([
        ...
        expect(psubs[2]
            .getPubSub()
            .getPeers()
            .map((s) => s.toString())).to.have.members([psubs[1].getPeerId().toString(), psubs[3].getPeerId().toString()]);
        const sendRecv = [];
    ...
    it('test gossipsub star topology with signed peer records', async function () {
        ...
        psubs[0].getPubSub().opts.D = 0;
        psubs[0].getPubSub().opts.Dhi = 0;
        psubs[0].getPubSub().opts.Dlo = 0;
        psubs[0].getPubSub().opts.Dscore = 0;
        // build the star
    ...
    it('test gossipsub direct peers', async function () {
        ...
        psubs[1].getPubSub().direct.add(psubs[2].getPeerId().toString());
        await connectPubsubNodes(psubs[1], psubs[2]);
        psubs[2].getPubSub().direct.add(psubs[1].getPeerId().toString());
    ...
    it('test gossipsub negative score', async function () {
        ...
        psubs.forEach((ps) => ps.getPubSub().subscribe(topic));
        await Promise.all(psubs.map(async (ps) => await awaitEvents(ps.getPubSub(), 'gossipsub:heartbeat', 3)));
        psubs.slice(1).forEach((ps) => ps.getPubSub().addEventListener('message', (evt) => {
    ...
    it('test gossipsub score validator ex', async function () {
        ...
        await connectPubsubNodes(psubs[1], psubs[2]);
        await connectPubsubNodes(psubs[0], psubs[2]);
        psubs[0].getPubSub().topicValidators.set(topic, async (topic, m, propagationSource) => {
            if (propagationSource.equals(psubs[1].getPeerId()))
                return MessageAcceptance.Ignore;
            if (propagationSource.equals(psubs[2].getPeerId()))
        ...
        await Promise.all(psubs.map(async (ps) => await awaitEvents(ps.getPubSub(), 'gossipsub:heartbeat', 2)));
        expect(psubs[0].getPubSub().score.score(psubs[1].getPeerId().toString())).to.be.eql(0);
        expect(psubs[0].getPubSub().score.score(psubs[2].getPeerId().toString())).to.be.lt(0);
    });
    ...
    it('test gossipsub opportunistic grafting', async function () {
        ...
        const connectPromises = real.map(async (psub) => await awaitEvents(psub.getConnectionManager(), 'peer:connect', 3));
        await connectSome(real, 5);
        await Promise.all(connectPromises);
        sybils.forEach((s) => {
            ;
            s.getPubSub().handleReceivedRpc = async function () {
                //
            };
        });
        ...
        await pRetry(async () => {
            for (const r of real) {
                const meshPeers = r.getPubSub().mesh.get(topic);
                if (meshPeers == null) {
