...
describe('gossipsub fallbacks to floodsub', () => {
        ...
        it('Dial event happened from nodeGs to nodeFs', async () => {
            ...
            await pRetry(() => {
                expect(nodeGs
                    .getPubSub()
                    .getPeers()
                    .map((s) => s.toString())).to.include(nodeFs.getPeerId().toString());
                expect(nodeFs
                    .getPubSub()
                    .getPeers()
                    .map((s) => s.toString())).to.include(nodeGs.getPeerId().toString());
            });
        });
    });
    ...
    describe('subscription functionality', () => {
        ...
        it('Subscribe to a topic', async function () {
            ...
            expect(nodeGs
                ...
                .map((p) => p.toString())).to.include(nodeFs.getPeerId().toString());
            expect(nodeFs
                .getPubSub()
                .getSubscribers(topic)
                .map((p) => p.toString())).to.include(nodeGs.getPeerId().toString());
            expect(nodeGs
                .getPubSub()
                .getPeers()
                .map((p) => p.toString())).to.include(changedPeerId.toString());
    ...
    describe('publish functionality', () => {
        ...
        it('Publish to a topic - nodeGs', async () => {
            const promise = pEvent(nodeFs.getPubSub(), 'message');
        ...
        it('Publish to a topic - nodeFs', async () => {
            const promise = pEvent(nodeGs.getPubSub(), 'message');
    ...
    describe('publish after unsubscribe', () => {
        ...
        it('Unsubscribe from a topic', async () => {
            const promise = pEvent(nodeFs.getPubSub(), 'subscription-change');
