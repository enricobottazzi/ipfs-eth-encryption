...
describe('gossip', () => {
    ...
    it('should send gossip to non-mesh peers in topic', async function () {
        ...
        const nodeASpy = nodeA.getPubSub();
        ...
        const mesh = nodeA.getPubSub().mesh.get(topic);
        if (mesh == null) {
    ...
    it('should send piggyback control into other sent messages', async function () {
        ...
        expect(nodeA
            ...
            .map((p) => p.toString())).to.include(peerB, "did not know about peerB's subscription to topic");
        // should be able to send them messages
        expect(nodeA.getPubSub().peers.get(peerB)).to.have.property('isWritable', true, 'nodeA did not have connection open to peerB');
        // set spy. NOTE: Forcing private property to be public
        const nodeASpy = sinon.spy(nodeA.getPubSub(), 'piggybackControl');
        // manually add control message to be sent to peerB
        const graft = { ihave: [], iwant: [], graft: [{ topicID: topic }], prune: [] };
        nodeA.getPubSub().control.set(peerB, graft);
        nodeA.getPubSub().gossip.set(peerB, []);
